#!/usr/bin/env python3
"""
Job Curator Bot - Postagem Canal PAGO
Baseado em post_next.py, mas para o canal premium.

Diferenças:
- Usa TELEGRAM_TOKEN_PAID / TELEGRAM_CHANNEL_PAID
- Fila separada (post_queue_paid.json)
- Mais vagas por dia (30 vs 5)
"""
import argparse
import json
import os
...
QUEUE_PATH = DATA_DIR / "post_queue_paid.json"  # Fila separada do FREE
POSTS_PATH = DATA_DIR / "telegram_posts.txt"     # Usa mesma fonte de posts
FAIL_STATE_PATH = DATA_DIR / "post_failures_paid.json"
PAUSE_FLAG_PATH = DATA_DIR / "posting_paused_paid.json"
...
def load_env():
    for path in (
        ...
        str(Path(__file__).parent / ".env.paid"),  # Arquivo separado para PAID
    ):
        try:
...
def send_telegram(text: str):
    """Envia para o canal PAGO"""
    token = os.environ.get("TELEGRAM_TOKEN_PAID")
    chat_id = os.environ.get("TELEGRAM_CHANNEL_PAID")
    
    if not token or not chat_id:
        raise RuntimeError("TELEGRAM_TOKEN_PAID/TELEGRAM_CHANNEL_PAID não configurados. Configure no .env ou .env.paid")
...
def send_alert(message: str):
    ...
    if str(chat_id).startswith("-100"):
        ...
        return
    url = f"https://api.telegram.org/bot{token}/sendMessage"
    payload = {"chat_id": chat_id, "text": f"⚠️ ALERTA VAGAS PAGO: {message}", "disable_web_page_preview": True}
    try:
        _send_via_requests(url, payload)
        return
    except Exception as e:
...
def pause_posting(reason: str):
    ...
    if not already_paused:
        ...
        send_alert(f"PAGO: Postagens pausadas (2 falhas seguidas). Motivo: {reason}")
    print("Postagens PAGO pausadas.")


def main():
    parser = argparse.ArgumentParser()
    parser.add_argument("--alert-test", action="store_true")
    parser.add_argument("--test", action="store_true", help="Testa envio sem avançar fila")
    args = parser.parse_args()
    load_env()
    
    if args.alert_test:
        send_alert("Teste de alerta (job-curator-bot PAGO).")
        print("OK: alerta enviado")
        return
    
    if PAUSE_FLAG_PATH.exists():
        print("Postagens PAGO pausadas (flag ativa).")
        return

    posts = load_posts()
    if not posts:
        print("Sem posts disponíveis.")
        send_alert("Sem posts disponíveis para canal PAGO.")
        return
    ...
    if prev_base and curr_base and prev_base == curr_base:
        ...
        if swap_idx is not None:
            ...
            save_posts(posts)
            print(f"[SWAP] Evitou repetição de {prev_base}: trocou posição {idx} com {swap_idx}")
    
    if args.test:
        print(f"[TEST] Vaga {idx+1}:")
        print(posts[idx][:200] + "...")
        return
    ...
    try:
        ...
        if state["count"] >= 2:
            ...
            send_alert(f"PAGO: Falha ao postar vaga {idx+1}: {e}")
        return
    ...
    queue["index"] = idx + 1
    save_queue(queue)
    print(f"OK PAGO: post {idx+1}/{len(posts)} enviado")
